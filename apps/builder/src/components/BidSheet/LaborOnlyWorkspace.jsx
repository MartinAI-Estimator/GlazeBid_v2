...
const LaborOnlyWorkspace = ({ system, setImportedSystems, onComplete, onBack, crewSize = 2, laborContingency = 2.5 }) => {
  ...
  // Live math for header badge + footer
  const baseHours        = tasks.reduce((sum, t) => sum + (Number(t.qty) || 0) * (Number(t.hrsPer) || 0), 0);
  const contingencyHours = baseHours * (laborContingency / 100);
  const daysOnsite       = Math.ceil(baseHours / (crewSize * 8)) || 0;
  const cleaningHours    = daysOnsite * 1;
  const totalHours       = baseHours + contingencyHours + cleaningHours;
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
            ...
            <span style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--accent-blue)' }}>
              ...
              <span style={{ fontSize: '0.9rem' }}>MHs</span>
            </span>
          </div>
          <button
            onClick={onComplete}
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <input
                          type="text"
                          placeholder="e.g., Demo existing storefront"
      ...
      <div style={{ padding: '1.5rem 2rem', background: 'var(--bg-deep)', borderTop: '2px solid var(--border-subtle)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', flexShrink: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', justifyContent: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            ...
            <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{baseHours.toFixed(1)} MHs</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            ...
            <span style={{ fontWeight: 700, color: 'var(--accent-blue)' }}>+{cleaningHours.toFixed(1)} MHs</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            <span>Labor Contingency ({laborContingency}%):</span>
            <span style={{ fontWeight: 700, color: '#f59e0b' }}>+{contingencyHours.toFixed(1)} MHs</span>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', background: 'var(--bg-card)', padding: '1.25rem', borderRadius: '10px', border: '1px solid var(--border-subtle)', textAlign: 'center' }}>
          ...
          <span style={{ fontSize: '2.25rem', fontWeight: 800, color: 'var(--accent-blue)', lineHeight: 1 }}>{totalHours.toFixed(1)} <span style={{ fontSize: '1rem', fontWeight: 600 }}>MHs</span></span>
        </div>
      </div>
    </div>
  );
...
export default LaborOnlyWorkspace;
